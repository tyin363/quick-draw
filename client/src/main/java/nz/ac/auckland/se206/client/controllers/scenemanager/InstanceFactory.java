...
public class InstanceFactory implements Callback<Class<?>, Object> {
  ...
  /**
   * Safely retrieve the instance associated with this type by wrapping the result of
   * {@link #get(Class)} in an {@code Optional}.
   *
   * @param type The type of the instance to retrieve
   * @param <T> The type of the instance
   ...
   * after the instance has been constructed. Any errors must be handled within the callback
   * itself.
   *
  ...
  /**
   * If the object is an instance of {@link EnableListener} then it will call the
   * {@link EnableListener#onEnable()} method.
   *
  ...
  /**
   * Iterates through all the cached singletons and if they're an instance of
   * {@link TerminationListener} then it invokes the {@link TerminationListener#onTerminate()}
   * method.
   */
